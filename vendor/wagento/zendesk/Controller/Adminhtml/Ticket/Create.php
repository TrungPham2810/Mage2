...
class Create extends Action
{
     ...
     * @param \Wagento\Zendesk\Helper\Data $helper
    ...
    public function execute()
    {
        ...
        if ($fieldId && isset($data['order_id']) && strlen(trim($data['order_id'])) > 0) {
            $ticket['custom_fields'][] = [
                'id' => $fieldId,
                'value' => $this->orderRepository->get($data['order_id'])->getIncrementId()
            ];
        }
        ...
        $this->messageManager->addErrorMessage(
            __("Error while creating new ticket, Make sure that connection was configured correctly.")
        );
        return $resultRedirect->setPath('*/*/edit');
    }
